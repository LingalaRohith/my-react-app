  ...
  function OrderSummary({isLoggedIn}) {
    ...
    const { movie, selectedSeats, ticketQuantities, showShowDates, showShowTimes} = location.state || { ticketQuantities: { adults: 0, children: 0, seniors: 0 }, selectedSeats: [], movie: {}, showShowDates: '', showShowTimes: '' };
    ...
    const subtotal = Object.keys(ticketQuantities).reduce((acc, key) => acc + (ticketQuantities[key] * ticketPrices[key]), 0);
    ...
    <div className="ticket-type-row">
      ...
      <div className="ticket-info">
        <span className="ticket-quantity">Quantity: {ticketQuantities.adults}</span>
        <span className="ticket-price">${(ticketQuantities.adults * ticketPrices.adults).toFixed(2)}</span>
      </div>
    </div>
    <div className="ticket-type-row">
      <p className="ticket-type">Child</p>
      <div className="ticket-info">
        <span className="ticket-quantity">Quantity: {ticketQuantities.children}</span>
        <span className="ticket-price">${(ticketQuantities.children * ticketPrices.children).toFixed(2)}</span>
      </div>
    </div>
    <div className="ticket-type-row">
      <p className="ticket-type">Senior</p>
      <div className="ticket-info">
        <span className="ticket-quantity">Quantity: {ticketQuantities.seniors}</span>
        <span className="ticket-price">${(ticketQuantities.seniors * ticketPrices.seniors).toFixed(2)}</span>
      </div>
    </div>
    {/* ... */}
  </div>
